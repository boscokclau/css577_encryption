...
########################################################################################################################
class CreateEncryptionKey(unittest.TestCase):
    def setUp(self):
    ...
    def test_unsupported_cipher(self):
        ...
        self.verify_create_encryption_key(data, expect, ValueError, "Cipher not supported: AES192")

    def test_unsupported_hmac_hash(self):
        data = {SECRET: "password", CIPHER: "aes256", HMAC_HASH: "sha384", KDF: "pbkdf2"}
        expect = {KEY: EXPECT_ENC_KEY_SHA512_32b}

        self.verify_create_encryption_key(data, expect, ValueError, "Hash not supported: SHA384")

    def test_unsupported_kdf(self):
        data = {SECRET: "password", CIPHER: "aes256", HMAC_HASH: "sha512", KDF: "bcrypt"}
        expect = {KEY: EXPECT_ENC_KEY_SHA512_32b}

        self.verify_create_encryption_key(data, expect, ValueError, "Unsupported kdf: bcrypt")

    ############################
    ## Verifiers
    ############################
    def verify_create_encryption_key(self, data: dict, expect: dict, exception_class_expect: BaseException,
                                     ex_msg: str):

        try:
            key = create_encryption_key(data[SECRET], data[CIPHER], data[HMAC_HASH], data[KDF])
